...
- Milestone 1 is in progress. The first slice implements a React/Konva viewport, pan/zoom/resize, grid and snapping controls, rectangle and polygon room proposals, stateless Shapely geometry validation, and undo/redo for accepted room additions. The session is in memory and refresh discards rooms. Selection/move/vertex editing, derived walls, constrained doors, and atomic persistence remain unfinished. See ADR-0009.
