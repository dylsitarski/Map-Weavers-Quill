...
- Milestone 2 is in progress. Its first increment adds SHA-256-addressed PNG assets in SQLite, one base-background raster layer, offline mock preview/accept/reject/regenerate, stale-preview rejection, undoable visibility/opacity and native save/reopen with generation provenance (ADR-0016). Cancellation discards the client proposal; persistent jobs/provider cancellation are not claimed. Room crops, binary polygon masks, enforced outside-mask preservation, independent room artwork and validated room/layer bindings are implemented (ADR-0017). Geometry edits clear affected art with undo. Artwork ordering, visibility and opacity controls are implemented with undo/redo and save/open. Flattened PNG/lossless WebP export is implemented at the canonical 480 × 320 raster resolution (ADR-0018), respecting accepted layer order/visibility/opacity. Durable queued generation, polling, idempotent cancellation and interrupted-job recovery are implemented (ADR-0019). Active synchronous mock computation is not forcibly interrupted; late output cannot publish. Browser preview recovery and real-provider interruption remain future work.
