...
- Milestone 2 is in progress. Its first increment adds SHA-256-addressed PNG assets in SQLite, one base-background raster layer, offline mock preview/accept/reject/regenerate, stale-preview rejection, undoable visibility/opacity and native save/reopen with generation provenance (ADR-0016). Cancellation discards the client proposal; persistent jobs/provider cancellation are not claimed. Room crops/masks, protected-pixel compositing, room imagery, independent artwork ordering and flattened export remain unfinished.
