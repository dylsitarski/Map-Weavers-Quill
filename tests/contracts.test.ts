...
const validate = ajv.compile<Project>(
  JSON.parse(readFileSync('packages/schema/project.schema.json', 'utf8')),
);
...
for (const fixture of ['two-rooms', 'all-entities']) {
  test(`${fixture} survives Python and TypeScript unchanged`, () => {
    const value: unknown = JSON.parse(
      readFileSync(`fixtures/projects/${fixture}.json`, 'utf8'),
    );
...
const errors = JSON.parse(
  readFileSync('fixtures/projects/invalid/expected-errors.json', 'utf8'),
...
for (const [name, path] of Object.entries(errors)) {
  test(`reject invalid fixture: ${name}`, () => {
    assert.equal(
      validate(
        JSON.parse(
          readFileSync(`fixtures/projects/invalid/${name}.json`, 'utf8'),
        ),
      ),
      false,
    );
    assert.ok(
      validate.errors?.some((e) => e.instancePath === `/${path.join('/')}`),
    );
  });
}
